...
8) store transaction hash as returned by SSN and show confirmation to user
...
The API will return a json object with the transaction details (on success), the transaction can take up to 5 seconds to be included in the blockchain. Should the connection be closed without receiving the response, the service needs to query his account transactions and find the transaction hash.

...
Tokens must be backed by an underlying FIAT reserve held in escrow with the cashier. Cashiers are responsible to redeem tokens for the underlying FIAT asset at the agreed rate with a settlement provider when the tokens are returned to the cashier.
...
Merchants on SSN, can configure their accounts to trust cashiers and assets they issue, cashiers can also configure assets to require approval by the cashier before a merchant account can hold the asset.
...
Payment provider who wants to offer both services should setup 2 asset issuing accounts, with different ```net_payout``` configurations for the service.
...
A one time payment is initiated by  merchant, prompting the user to click on a payment link which will call the corresponding endpoint at the cashier. The cashier must authenticate the user or provide an environment where the user is already authenticated (e.g. deep-link to a mobile application) and ask the user to agree to the charge.
...
Cashiers can of course also implement recurring payments as scheduled payment setup by user and agreed upon with service providers. The Cashiers could periodically query the service provider for any open invoices for a particular customer, and when the response returns a payment due, the Cashiers can simply issue the payment authorization on SSN.

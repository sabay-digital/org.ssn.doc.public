...
For recurring payments, the cashier need to store the details of the request and periodically execute the same payment on behalf of the user. The payment provider may implement a service to allow his user to manage the recurring payments via the payment provider own service interface, e.g. setup, suspend or cancel the payment. 
...
Service providers may reject a renewed payment for a old subscription by sending the payment back, see refunds. Service providers may also cancel the subscription after payments have been missed.
...
If a merchant receives a payment which cannot be processed or where the user has cancels the order after the payment was made, the merchant can refund the payment by reversing the transaction. Refunds are implemented by sending the received amount back to the originating sender address and referencing the original transaction hash in the memo field.

...
This document describes the function and services provided by payment providers (cashiers) on SSN.
...
A onetime payment is initiated by the merchant, prompting the user to click on a payment link which will call the corresponding endpoint at the payment provider. The payment provider must authenticate the user or provide an environment where the user is already authenticated (e.g. deep-link to a mobile application) and ask the user to agree to the charge.

## Pre-authorized payment

A pre-authorized payment involves 2 steps:

1) setting up the authorization
2) requesting payment using the authorization established

A pre-authorized payment is initiated from the merchant, using the credentials stored with the merchant and calling the API endpoint at the payment provider. The payment provider will execute the charge instantly, without asking the user for authorization. The service may inform the user about to charge by sending notification via a different channel. 
...
Recurring payments involve 2 steps:

1) setting up the recurring charge (merchant initiated)
2) making the payment (payment provider driven)

Recurring payments are setup by the merchant with the payment provider, but executed by the payment provider on a regular scheduled base.

During the setup the payment provider must ask the user for authorization and confirm the recurring charge. On the charing date the payment provider must issue the tokens on SSN and book the amount from the user balance to the escrow account, if the user has the required funds in his wallet.

Depending on the particulars of the payment provider, the charge may not always occurs on the same date. For example a recurring charge is setup with a mobile wallet provider, but the user did not top-up the wallet on time. In this case transaction maybe execute at a later date when the user tops up his wallet. The service provider is responsible to take delayed recurring payments into considerations when implementing the service.

Payment providers can of course also implement recurring payments as scheduled payment setup by user and agreed upon with service providers. The payment provider could periodically query the service provider for any open invoices for a particular customer and when the response returns a payment due, the payment provider can issue the payment authorization on SSN.

## Direct Payment

A direct payment occurs when the user is using the payment providers own software, e.g. mobile app or online banking software and either scans a QR code enters a payment address directly.

The SSN payment address resolver can resolve the payment address and return the billing amount and other details. The payment provider will issue tokens on SSN based on the data provided by the lookup and confirmation from the user.

# Implementing a cashier
...
* `/charge/onetime/{payment_address}` a endpoint for onetime payment authorization
...
and may implement recurring payments by implementing:
...
Direct cashiers have no public API to access. The merchant will not call the cashier service but can use a payment address or QR code to transmit the payment information to the payment provider.
...
1) merchant will assign the user a key pair which is comprised of a public key and private key, 
2) merchant will store the private key with the user profile
3) merchant will redirect user to payment provider and submitting the public key to cashier API
4) payment provider will verify the user
5) payment provider will add the public key to the profile of the user who has authenticated himself

The user can then be identified by the cashier with this public key. For subsequent payment request using the pre-authorization API the request will be signed by the private key store with the merchant. The signature can be validated with the public key stored with the payment provider.

A user profile with a payment provider may have more then one public key stored, if the user has pre-authorized payments with different merchants or service providers.

The cashier may enforce transaction limits for pre-authorized payments and publish them in its info profile. The API to implement the service is described in SSN 3rd party API for the endpoint
...
3. redirect to caller based one the header callback URL

## Recurring payment

For recurring payments, the cashier needs to store the details of the request and periodically execute the same payment on behalf of the user. The payment provider may implement a service to allow his user to manage the recurring payments via the payment providers own service interface, e.g. setup, suspend or cancel the payment. 
...
Service providers may reject a renewed payment for an old subscription by sending the payment back, see refunds. Service providers may also cancel the subscription after payments have been missed.

Recurring payments on SSN are represented in frequency; daily, weekly, monthly, yearly with a numeric qualifier for the interval that describes how often the payment is made. The example below shows a recurring payment that occurs once every 2 weeks.

  ```yaml
  recurring_payment_frequency: w
  recurring_payment_interval: 2
  recurring_payment_start: YYYY-MM-DD
  ```

if the recurring_payment_interval is not present, a value of 1 should be assumed. If recurring_payment_start is not present, todays date should be used.
...
1. resolve payment address from service provider (see SSN Payment Address resolver API)
2. Check and valid input received (including data received from step 1)
3. verify if merchant network account is able to receive the payment (validate trustline, see SSN 3rd party API)
...
The payment provider can verify that the secret key linked to the public key stored with the payment provider has signed the request. The SSN 3rd party API has a function to valid signatures, however the payment provider needs to validate that the hash corresponds to the URI using the sha256 hashing function.
...
**If the network address or memo is missing or the resolver requests ends in error, the payment provider must exit the process**, inform the user and inform the service using the call-back URL if provided.
...
6. show user success message with SSN transaction hash

The payment provider has at this stage received the SSN transaction hash, it must present it to the user in a way in which the user can easily capture the hash via screenshot or via a link which the user can save. The transaction hash proves to the user that the payment was done and he can use this ID in any dispute with the merchant to verify he made the payment.
...
4. SSN returns transaction hash or rejects transaction

Once the transaction hash has been received it should be shown to the user and added to the permanent record for the payment. The cashier can also build the transaction using the stellar SDK as published by the SDF. 

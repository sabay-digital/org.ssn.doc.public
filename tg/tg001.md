...
A one time payment is initiated by  merchant, promoting the user to click on a payment link which will call the corresponding endpoint at the cashier. The cashier must authenticate the user or provide a environment where the user is already authenticated (e.g. deep-link to a mobile application) and ask the user to agree to the charge.
